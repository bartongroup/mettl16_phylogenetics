    ...
    with open(gtf_fn) as gtf:
        for i, record in enumerate(gtf):
            ...
            if feat_type == 'transcript':
                ...
                try:
                    prot_id = get_gtf_attribute(record, 'protein_id')
                except ValueError:
                    # not a protein coding gene
                    continue
                transcript_to_protein_mapping[transcript_id] = prot_id
            elif feat_type == 'CDS':
                try:
                ...
                try:
                    ...
                    # some gtfs only have protein_id label on transcript lines
                    # these (should) always come before CDS lines
                    transcript_id = get_gtf_attribute(record, 'transcript_id')
                    try:
                        prot_id = transcript_to_protein_mapping[transcript_id]
                    except KeyError:
                        warnings.warn(f'Theres something quite dodgy about "{transcript_id}"')
                        continue
                idx = (chrom, gene_id, prot_id, strand)
                if idx not in gtf_records:
                    gtf_records[idx] = []
                gtf_records[idx].append([start, end])
    for (chrom, gene_id, prot_id, strand), invs in gtf_records.items():
        invs.sort(key=itemgetter(0))
        cds = np.array(invs)
        yield chrom, gene_id, prot_id, strand, cds
...
def gtf_to_protein_introns(gtf_fn, output_bed_fn):
    with open(output_bed_fn, 'w') as bed:
        for chrom, gene_id, prot_id, strand, cds in gtf_cds_parser(gtf_fn):
